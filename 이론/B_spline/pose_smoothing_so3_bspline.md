...
(N은 차수가 늘어날 수록 knot 범위 증가, (0차에서 ui~ui+1, 1차에선 ui~ui+2), N은 기존 값으로 정의되는 기저 (basis), 제어점을 통해 spline 제어)즉 곡선은 **basis 함수들의 가중합**이고, 가중치가 제어점이다.

### 1.2 주요 성질

- **국소 지지(local support)**: $N_{i,p}$ 는 $[u_i,u_{i+p+1})$ 에서만 0이 아님. → 제어점 하나를
  움직이면 곡선의 **국소 구간만** 변한다(전역 영향 없음(제어점을 지나는 basis들에만 영향)). 스플라인 스무딩이 안정적인 이유.
